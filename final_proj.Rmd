...
mass_dist <- mass |> ggplot(aes(x = `mass (g)`)) + 
...
print(mass_dist)
#ggsave("mass_dist.png")


Massfall <- mass |> ggplot(aes(x = `mass (g)`, fill = fall)) +
...
class_box <- mass_filtered |> ggplot(aes(x = class, y = log10(`mass (g)`))) +
  geom_boxplot() +
  theme_bw()
  # gives boxplot for the range in mass for the top 10 classes. CM2 has the lowest mean mass, L4 has the highest, and H5 has the biggest range
print(class_box)
#ggsave("class_box.png")
...
# maps all meteorites and all top 10 class meteorites in the us
