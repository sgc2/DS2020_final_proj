...
#mass_filtered |> count(class) |> rename(count = n) |> arrange(desc(count)) |> mutate(`Percent Makeup` = (count/45566)*100)
...
# |> ggplot(aes(x = year, y = count, fill = class)) + geom_point() + 
  #coord_cartesian(xlim = c(1700, 2025)) + 
  #cale_fill_discrete(name = "Meteor Within Top 10 Classes")

  
#Meteorite |> 
 #filter(year>=860 & year<=2016) |> ggplot(aes(x = year, y = `mass (g)`)) + 
  #geom_point() +
 # ggtitle("Distribution of the meteorite landings over time") + 
  #coord_cartesian(xlim = c(1750, 2025), ylim =c(0, 40000000))
